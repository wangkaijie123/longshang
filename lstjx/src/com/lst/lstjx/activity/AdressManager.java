...
public class AdressManager extends Activity implements AddressCallback {
		...
		public void handleMessage(android.os.Message msg) {
			...
			case ADRESSDETAIL:
				...
				if (mRecListBean.size()!=0&&mRecListBean!=null) {
					...
					mReceiptAdapter = new ReceiptAdapter(AdressManager.this, mRecListBean,
							AdressManager.this);
	...
	private void initView() {
		...
		haveaddress = (RelativeLayout) findViewById(R.id.haveaddress);
		...
		addAddress = (Button)findViewById(R.id.receipt_address_BtId);
	}
	...
	public void click(View v) {
		 ...
		 * 删除收货地址
		 */
		case R.id.item_receipt_decelete:
		...
		case R.id.receipt_moren_RlId:
			 ...
			 * 编辑收货地址
			 */
		case R.id.item_receipt_editId:
			 mPosition = (Integer) v.getTag();
			 String id = mRecListBean.get(postition).getId();
			 String accept_name = mRecListBean.get(postition)
						.getAccept_name();
			AdressDetail mAdressDetail = new AdressDetail();
			String zip = mRecListBean.get(postition).getZip();
			String mobile = mRecListBean.get(postition).getMobile();
			String adress = mRecListBean.get(postition).getAdress();
			Intent intent= new Intent(AdressManager.this,AddAdressActivity.class);
			intent.putExtra("Id", id);
			intent.putExtra("AcceptName", accept_name);
