...
import com.lst.lstjx.activity.ContactListActivity.ListAdapter;
...
/**
 * author describe 会话列表界面 menu 点击发起聊天 跳入联系人选择列表 parameter return
 */
public class ChooseFriendToChat extends Activity implements OnClickListener {
	protected static final int CREATEFRIENDGETFRIENDLIST = 0X65456;
	private TextView choose_friend_to_chat_cancel,
			choose_friend_to_chat_confirm;
	...
	private String userid, markname;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.ac_choose_friend_to_chat);
		initView();
		requstFriendList();
	}
	...
	private void initView() {
		// TODO Auto-generated method stub
		userid = SharePrefUtil.getString(ChooseFriendToChat.this, "userId", "");
		choose_friend_to_chat_cancel = (TextView) findViewById(R.id.choose_friend_to_chat_cancel);
		choose_friend_to_chat_confirm = (TextView) findViewById(R.id.choose_friend_to_chat_confirm);
		choose_friend_to_chat_cancel.setOnClickListener(this);
		choose_friend_to_chat_confirm.setOnClickListener(this);
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		switch (v.getId()) {
		case R.id.choose_friend_to_chat_cancel:
			finish();
			break;
		case R.id.choose_friend_to_chat_confirm:
			// 判断第几个是true 在做if判断，如果是true就装到useridlist中

			System.out.println("asddddaaaaaaaaaaaaaaa" + mChecked.size());
			for (int i = 0; i < mChecked.size(); i++) {
				if (mChecked.get(i)) {

					Fid = mGetFriendBean.data.get(i).getFid();
					if (mGetFriendBean.data.get(i).getMark().toString().trim() != null
							&& !"".equals(mGetFriendBean.data.get(i).getMark().toString().trim())) {

						markname = mGetFriendBean.data.get(i).getMark()
								.toString().trim();
					}else {
						
						username = mGetFriendBean.data.get(i).getUsername()
								.toString().trim();
					}

					System.out.println(mGetFriendBean.data.get(i).getFid());
				}
			}
			...
			if (fidList.size() == 1) {
				if (markname != null && !"".equals(markname)) {
					RongIM.getInstance().startPrivateChat(
							ChooseFriendToChat.this, Fid, markname);
					System.out.println("=====我的聊天界面title======"+markname);
				} else {

					RongIM.getInstance().startPrivateChat(
							ChooseFriendToChat.this, Fid, username);
					System.out.println("=====我的聊天界面title======"+username);
				}
			} else {
				Toast.makeText(ChooseFriendToChat.this, "请选择一位好友",
	...
	private void requstFriendList() {
		...
		http.post(ChooseFriendToChat.this, ConstantsUrl.getFriendList, params,
