...
public class MyWlFragment extends Fragment implements OnClickListener {
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 3:
				if (delDynamicCode == 1) {
					WinToast.makeText(getActivity(), "删除成功").show();
					arrayList.remove(pos);
					adapter = new MyWlAdapter(getActivity(), arrayList);
					mPullRefreshListView.setAdapter(adapter);
				} else {
					WinToast.makeText(getActivity(), "删除失败,请重试").show();
				}
				break;
			default:
				break;
	...
	private void initView(View v) {
		userid = SharePrefUtil.getString(getActivity(), "userId", null);
	...
	private void myDynamicDel(int position) {
		...
		http.post(getActivity(), ConstantsUrl.my_dynamic_del, params,
